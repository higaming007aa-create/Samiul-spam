...
app = FastAPI(title="Samiul ULTRA SPAM - Dashboard")
...
def load_users():
    if not os.path.exists(USERS_FILE):
        default_users = {
            "Samiul": {"password": "samiul", "role": "admin"}
        }
    ...
    try:
        ...
        return {"Samiul": {"password": "samiul", "role": "admin"}}
    ...
    users.forEach(u => {
      listCont.innerHTML += `
        <div style="background: rgba(255,255,255,0.05); padding: 5px 10px; border-radius:6px; display:flex; align-items:center; gap:10px; border:1px solid rgba(255,255,255,0.1)">
          ...
          ${u.username !== "Samiul" ? `<button style="background:none; border:none; color:red; cursor:pointer;" onclick="deleteUser('${u.username}')">❌</button>` : ''}
        </div>
      `;
    });
  }
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🔥 Samiul ULTRA SPAM SYSTEM STABLE")
    print("🚀 Auto Multi-User Mode Activated")
    print("📡 Default Admin -> User: Samiul | Pass: samiul")
    print("="*60 + "\n")
